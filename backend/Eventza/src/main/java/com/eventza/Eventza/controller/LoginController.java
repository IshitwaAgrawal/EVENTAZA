...
public class LoginController {
  ...
  public ResponseEntity<?> createLoginToken(@RequestBody LoginRequest request) throws Exception {
    ...
    if (userDetails.isEnabled()) {
      User user = userService.getUserByUsername(request.getUsername());
      ResponseUser r_user = new ResponseUser(user.getId(), user.getUsername(), user.getName(),
          user.getEmail(), user.getRoles(), user.isNewsletter_service(), user.isOrganizer(),
          user.getRegisteredEvents(), user.getHostedEvents(), user.getWishlist());
